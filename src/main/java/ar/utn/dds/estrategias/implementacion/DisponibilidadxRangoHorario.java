...
public class DisponibilidadxRangoHorario implements EstrategiaDisponibilidad {
	...
	public Boolean estaDisponible(POI poi,List<Servicio> servicios, String _nombreServicio,LocalDateTime _horarioConsultado) {
		...
		return poi.getJornadaDisponible().stream().anyMatch((jornada)->jornada.DentroHorarioDeRango(_horarioConsultado.getDayOfWeek(), _horarioConsultado.toLocalTime()));
		
	}
	
	public Boolean estaDisponible(Servicio servicio,LocalDateTime _horarioConsultado) {
				
		return servicio.getJornadaDisponible().stream().anyMatch((jornada)->jornada.DentroHorarioDeRango(_horarioConsultado.getDayOfWeek(), _horarioConsultado.toLocalTime()));
